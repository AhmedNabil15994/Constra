    ...
    <form class="form" method="post" action="{{ URL::to($designElems['mainData']['url'].'/create') }}" enctype="multipart/form-data">
        ...
        <div class="card-body">
            <div class="form-group row p-0 m-0 mb-5">
                ...
                <div class="col-9">
                    <input type="text" class="form-control" name="first_name" value="{{old('first_name')}}" placeholder="{{ trans('User::user.form.first_name') }}">
                </div>
            </div>
            <div class="form-group row p-0 m-0 mb-5">
                <div class="col-3">
                ...
                <div class="col-9">
                    <input type="text" class="form-control" name="last_name" value="{{old('last_name')}}" placeholder="{{ trans('User::user.form.last_name') }}">
                </div>
            </div>
            ...
            <div class="form-group row p-0 m-0 mb-5">
                ...
                <div class="col-9">
                    <input type="email" class="form-control" name="email" value="{{old('email')}}" placeholder="{{ trans('User::user.form.email') }}">
                </div>
            </div>
            ...
            <div class="form-group row p-0 m-0 mb-5">
                ...
                <div class="col-9">
                    <select class="form-control" data-toggle="select2" data-placeholder="{{trans('Dashboard::dashboard.choose')}}" id="kt_select2_3" name="extra_permissions[]" multiple="multiple">
                        @foreach($permissions as $key => $permission)
                        @php $module = str_replace('Controller','',$key);@endphp 
                        <optgroup label="{{ trans($module.'::'.lcfirst($module).'.title') }}">
                            ...
                            @endphp 
                            <option value="{{ $onePerm['perm_name'] }}">{{ $perm }}</option>
                            @endforeach
                        </optgroup>
                        @endforeach
                    </select>
