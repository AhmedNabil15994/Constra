...
<div class="card card-custom formNumbers">
    ...
    <form class="form" method="post" action="{{ URL::to($designElems['mainData']['url'].'/update/'.$page->id) }}" enctype="multipart/form-data">
        @csrf
        <div class="card-body">
            ...
            <div class="tab-content mt-5" id="myTabContent">
                ...
                <div class="tab-pane fade  {{$lang['prefix'] == LANGUAGE_PREF? 'active show' : ''}}" id="home-{{$key+1}}" role="tabpanel" aria-labelledby="home-tab-{{$key+1}}">
                    <div class="form-group row p-0 m-0 mb-5 pt-5">
                        <div class="col-3">
                            <label for="inputEmail3">{{ trans('Role::role.form.name') }} :</label>                        
                        </div>
                        <div class="col-9">
                            <input type="text" class="form-control" name="name_{{$lang['prefix']}}" value="{{ $page->{'name_'.$lang['prefix']} }}" placeholder="{{ trans('Role::role.form.name') }}">
                        </div>
                    </div>
                </div>
                @endforeach
            ...
            <div class="form-group row p-0 m-0 mb-5">
                ...
                <div class="col-9">
                    <span class="switch switch-icon">
                        <label>
                            <input type="checkbox" value="1" name="status" {{$page->status  == 1 ? 'checked' : ''}}/>
                            <span></span>
                        </label>
                    </span>
                </div>
