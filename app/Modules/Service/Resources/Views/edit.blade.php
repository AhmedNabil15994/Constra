...
@include('dashboard.Layouts.breadcrumb',[
    'breadcrumbs' => [
        ...
        [
            'title' => $service->{'name_'.LANGUAGE_PREF},
            'url' => \URL::current()
        ],
    ]
])
...
<div class="card card-custom formNumbers">
    ...
    <form class="form" method="post" action="{{ URL::to($designElems['mainData']['url'].'/update/'.$service->id) }}" enctype="multipart/form-data">
        @csrf
        <div class="card-body">
            ...
            <div class="tab-content mt-5" id="myTabContent">
                ...
                <div class="tab-pane fade  {{$lang['prefix'] == LANGUAGE_PREF? 'active show' : ''}}" id="home-{{$key+1}}" role="tabpanel" aria-labelledby="home-tab-{{$key+1}}">
                    <div class="form-group row p-0 m-0 mb-5 pt-5">
                        ...
                        <div class="col-9">
                            <input type="text" class="form-control" name="name_{{$lang['prefix']}}" value="{{ $service->{'name_'.$lang['prefix']} }}" placeholder="{{ trans('Role::role.form.name_'.$lang['prefix']) }}">
                        </div>
                    </div>
                </div>
                @endforeach
            ...
            <div class="form-group row p-0 m-0 mb-5">
                ...
                <div class="col-9">
                    <input type="text" class="form-control" name="icon" value="{{ $service->icon }}" placeholder="{{ trans('Service::service.form.icon') }}">
                </div>
            </div>
            <div class="form-group row p-0 m-0 mb-5">
                <div class="col-3">
                ...
                <div class="col-9">
                    <span class="switch switch-icon">
                        <label>
                            <input type="checkbox" value="1" name="status" {{$service->status  == 1 ? 'checked' : ''}}/>
                            <span></span>
                        </label>
                    </span>
                </div>
