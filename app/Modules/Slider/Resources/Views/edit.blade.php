...
@include('dashboard.Layouts.breadcrumb',[
    'breadcrumbs' => [
        ...
        [
            'title' => $slider->{'title_'.LANGUAGE_PREF},
            'url' => \URL::current()
        ],
    ]
])
...
<div class="card card-custom formNumbers">
    ...
    <form class="form" method="post" action="{{ URL::to($designElems['mainData']['url'].'/update/'.$slider->id) }}" enctype="multipart/form-data">
        @csrf
        <div class="card-body">
            ...
            <div class="tab-content mt-5" id="myTabContent">
                ...
                <div class="tab-pane fade  {{$lang['prefix'] == LANGUAGE_PREF? 'active show' : ''}}" id="home-{{$key+1}}" role="tabpanel" aria-labelledby="home-tab-{{$key+1}}">
                    <div class="form-group row p-0 m-0 mb-5 pt-5">
                        ...
                        <div class="col-9">
                            <input type="text" class="form-control" name="title_{{$lang['prefix']}}" value="{{ $slider->{'title_'.$lang['prefix']} }}" placeholder="{{ trans('Section::section.form.title_'.$lang['prefix']) }}">
                        </div>
                    </div>
                    <div class="form-group row p-0 m-0 mb-5 pt-5">
                        <div class="col-3">
                        ...
                        <div class="col-9">
                            <textarea  class="form-control" name="description_{{$lang['prefix']}}" placeholder="{{ trans('Section::section.form.description_'.$lang['prefix']) }}" cols="30" rows="10">{{ $slider->{'description_'.$lang['prefix']} }}</textarea>
                        </div>
                    </div>
                </div>
                @endforeach
            ...
            <div class="form-group row p-0 m-0 mb-5">
                ...
                <div class="col-9">
                    <select name="page_id" class="form-control" data-toggle="select2">
                        ...
                        <option value="{{$page->id}}" {{$page->id == $slider->page_id ? 'selected' : ''}} >{{$page->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="form-group row p-0 m-0 mb-5">
                ...
                <div class="col-9">
                    <span class="switch switch-icon">
                        <label>
                            <input type="checkbox" value="1" name="status" {{$slider->status  == 1 ? 'checked' : ''}}/>
                            <span></span>
                        </label>
                    </span>
                </div>
            ...
            <div class="form-group row p-0 m-0 mb-5">
                ...
                <div class="col-9">
                    <div class="image-input image-input-outline" id="kt_image_5" style="background-image: url({{ $slider->image ? $slider->image_url : asset(config('modules.site_configs.default_upload_img'))  }})">
