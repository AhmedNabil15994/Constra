...
@include('dashboard.Layouts.breadcrumb',[
    'breadcrumbs' => [
        ...
        [
            'title' => $category->{'name_'.LANGUAGE_PREF},
            'url' => \URL::current()
        ],
    ]
])
...
<div class="card card-custom formNumbers">
    ...
    <form class="form" method="post" action="{{ URL::to($designElems['mainData']['url'].'/update/'.$category->id) }}" enctype="multipart/form-data">
        @csrf
        <div class="card-body">
            ...
            <div class="tab-content mt-5" id="myTabContent">
                ...
                <div class="tab-pane fade {{$lang['prefix'] == LANGUAGE_PREF? 'active show' : ''}}" id="home-{{$key+1}}" role="tabpanel" aria-labelledby="home-tab-{{$key+1}}">
                    <div class="form-group row p-0 m-0 mb-5 pt-5">
                        <div class="col-3">
                            <label for="inputEmail3">{{ trans('Category::category.form.name_'.$lang['prefix']) }} :</label>                        
                        </div>
                        <div class="col-9">
                            <input type="text" class="form-control" name="name_{{$lang['prefix']}}" value="{{ $category->{'name_'.$lang['prefix']} }}" placeholder="{{ trans('Category::category.form.name_'.$lang['prefix']) }}">
                        </div>
                    </div>
                </div>
                @endforeach
            ...
            <div class="form-group row p-0 m-0 mb-5 pt-5">
                <div class="col-3">
                    <label for="inputEmail3">@lang("Category::category.form.sort") :</label>                        
                </div>
                <div class="col-9">
                    <input type="number" class="form-control" name="sort" value="{{$category->sort}}" placeholder="{{__("Category::category.form.sort")}}">
                </div>
            </div>

            <div class="form-group row p-0 m-0 mb-5">
                <div class="col-3">
                    <label for="inputEmail3">{{ trans('Category::category.form.parent') }} :</label>
                </div>
                <div class="col-9">
                    <select name="parent_id" class="form-control" data-toggle="select2">
            ...
            <div class="form-group row p-0 m-0 mb-5">
                <div class="col-3">
                    <label for="inputEmail3">{{ trans('Category::category.form.status') }} :</label>
                </div>
                <div class="col-9">
                    <span class="switch switch-icon">
                        <label>
                            <input type="checkbox" value="1" name="status" {{ $category->status  == 1 ? 'checked' : ''}}/>
                            <span></span>
                        </label>
                    </span>
                </div>
            ...
            <div class="form-group row p-0 m-0 mb-5">
                ...
                <div class="col-9">
                    <div class="image-input image-input-outline" id="kt_image_5" style="background-image: url({{ $category->image ? $category->image_url : asset(config('modules.site_configs.default_upload_img'))  }})">
