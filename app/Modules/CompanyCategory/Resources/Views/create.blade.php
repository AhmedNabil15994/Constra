    ...
    <form class="form" method="post" action="{{ URL::to($designElems['mainData']['url'].'/create') }}" enctype="multipart/form-data">
        ...
        <div class="card-body">
            ...
            <div class="tab-content mt-5" id="myTabContent">
                ...
                <div class="tab-pane fade {{$key == 0 ? 'active show' : '' }}" id="home-{{$key+1}}" role="tabpanel" aria-labelledby="home-tab-{{$key+1}}">
                    <div class="form-group row p-0 m-0 mb-5 pt-5">
                        ...
                        <div class="col-9">
                            <input type="text" class="form-control" name="name_{{$lang['prefix']}}" value="{{old('name_'.$lang['prefix'])}}" placeholder="{{ trans('Category::category.form.name_'.$lang['prefix']) }}">
                        </div>
                    </div>
                </div>
                @endforeach
            ...
            <div class="form-group row p-0 m-0 mb-5 pt-5">
                ...
                <div class="col-9">
                    <input type="number" class="form-control" name="sort" value="{{old('sort')}}" placeholder="{{__("Category::category.form.sort")}}">
                </div>
            </div>
                ...
                <div class="col-9">
                    <select name="parent_id" class="form-control" data-toggle="select2">
                        ...
                        @foreach($parents as $parent)
                            <option value="{{$parent->id}}" {{$parent->id == old('parent_id') ? 'selected' : ''}} >{{$parent->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
