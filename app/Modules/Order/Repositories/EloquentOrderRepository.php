...
use App\Http\Requests\OrderValidator as ModelValidator;

class EloquentOrderRepository implements EntityRepositoryInterface {
    ...
    public function dataList($request){
        ...
        if(isset($input['service_id']) && !empty($input['service_id'])){
            $source = $source->where('service_id',$input['service_id']);
        }        
