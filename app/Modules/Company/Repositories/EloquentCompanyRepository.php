...
use App\Http\Requests\CompanyValidator as ModelValidator;

class EloquentCompanyRepository implements EntityRepositoryInterface {
    ...
    public function dataList($request){
        ...
        if(isset($input['location']) && !empty($input['location'])){
            $source = $source->where('location','LIKE','%'.$input['location'].'%');
        }
    ...
    public function create($request) {
        ...
        if($request->hasFile('image')){
            $image = \FilesHelper::uploadFile('companies',$request->file('image'),$return->id);
    ...
    public function update($request,$id) {
        ...
        if($request->hasFile('image')){
            $updateData['image'] = \FilesHelper::uploadFile('companies',$request->file('image'),$id);
        }else{
    ...
    public function delete($id) {
        ...
        if ($record->trashed()){
            \FilesHelper::deleteDirectory(public_path('/').'/uploads/companies/'.$id);
