				...
				columnDefsVar.push({
					...
					render: function(data, type, full, meta) {
						return '<span class="switch switch-icon">'+
                        		'<label>'+
                        			"<input type='checkbox' class='form-control' data-area='"+full.id+"' data-tabs='status'   "+(full.status == 1 ? 'checked' : '')+"/>"+
		                            '<span></span>'+
		                        '</label>'+
		                    '</span>';

						'<a class="'+item['anchor-class']+' '+labelClass+'" data-col="'+item['data-col']+'" data-id="'+full.id+'">'+data+'</a>';
					},
				});
			}else{
				columnDefsVar.push({
			...
			columnDefsVar.push({
				...
				render: function(data, type, full, meta) {
					...
					if($('input[name="data-area"]').val() == 1){
						editButton = '<a data-toggle="tooltip" data-original-title="'+editText+'" href="/'+designElems.mainData.url+'/edit/'+data+'" class="mx-1 action-icon btn btn-sm btn-primary btn-icon"> <i class="icon-xl la la-edit"></i></a>';
					}
